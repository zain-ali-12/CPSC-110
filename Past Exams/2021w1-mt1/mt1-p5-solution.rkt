...
(check-expect (render-cat (make-cat LEFT 2 false))
              (place-image UNHAPPY-CAT-IMAGE LEFT CAT-Y MTS))

(check-expect (render-cat (make-cat RIGHT  -3 false))
              (place-image UNHAPPY-CAT-IMAGE RIGHT CAT-Y MTS))

(check-expect (render-cat (make-cat RIGHT -3 true))
...
(define (render-cat c)
  (place-image (if (cat-happy? c) HAPPY-CAT-IMAGE UNHAPPY-CAT-IMAGE)
               (cat-x c) CAT-Y
               MTS))
...
;; Your design must include all applicable HtDF recipe elements including
;; @signature and @template-origin tags.
;; Note two very important points:
;;   - the cat should turn around if it comes = to the wall or tries to
;;     go past the wall, AND
;;   - in the tick when a cat turns around it should stay right at the
;;     wall, it should not come off the wall.
;;
...
(check-expect (tock-cat (make-cat MID  2 false)) (make-cat (+ MID  2)  2 false))

;; left edge
(check-expect (tock-cat (make-cat (+ LEFT 2) -3 false)) (make-cat LEFT 3 false))
(check-expect (tock-cat (make-cat (+ LEFT 3) -3 false)) (make-cat LEFT 3 false))
(check-expect (tock-cat (make-cat (+ LEFT 4) -3 false))
              (make-cat (+ LEFT 1) -3 false))

;; right edge
(check-expect (tock-cat (make-cat (- RIGHT 2) 3 false)) (make-cat RIGHT -3 false))
(check-expect (tock-cat (make-cat (- RIGHT 3) 3 false)) (make-cat RIGHT -3 false))
(check-expect (tock-cat (make-cat (- RIGHT 4) 3 false))
...
(define (tock-cat c)
  (if (cat-happy? c)
      c
      (cond [(<= (+ (cat-x c) (cat-unhappy-vel c)) LEFT)
             (make-cat LEFT
                       (- (cat-unhappy-vel c))
                       (cat-happy? c))]
