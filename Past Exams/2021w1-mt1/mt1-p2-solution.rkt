...
;; produce true if x < y < z

;(define (in-order? x y z) false) ;stub

(check-expect (in-order? 1 2 3) true)
(check-expect (in-order? 5 5 8) false) ; x = y
(check-expect (in-order? 3 5 5) false) ; y = z
(check-expect (in-order? 4 2 6) false) ; x !< y
(check-expect (in-order? 5 8 7) false) ; y !< z
