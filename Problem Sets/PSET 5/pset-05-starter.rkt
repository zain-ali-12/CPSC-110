...
(define (main g)
  (big-bang g
    (on-draw   render-balls)   ;Game -> Image
...
(check-expect (render-balls (make-game empty 300)) (place-image PADDLE
                                                                300
                                                                PADDLE-CTR-Y
                                                                MTS))

(check-expect (render-balls (make-game
                             (cons (make-ball 10 20 3 4)
...
(check-expect (render-balls (make-game
                             (cons (make-ball 30 40 4 3) empty) 300))
              (place-ball (make-ball 30 40 4 3) (place-image PADDLE
                                                             300
                                                             PADDLE-CTR-Y
                                                             MTS)))
...
(@template
 (define (render-balls g)
   (cond [(empty? (game-balls g)) (...)]
         [else
          (... (fn-for-ball (game-balls g))
               (render-balls (game-balls g)))])))

(define (render-balls g)
  (cond [(empty? (game-balls g)) (place-image
                                  PADDLE
                                  (game-paddle g)
                                  PADDLE-CTR-Y
                                  MTS)]
        [else
         (place-ball (first (game-balls g))
                     (render-balls (make-game (rest (game-balls g))
                                              (game-paddle g))))]))
...
(check-expect (next-balls (make-game empty 300)) (make-game empty
                                                            300))
(check-expect (next-balls (make-game
...
(check-expect (next-balls (make-game
                           (cons (make-ball (+ LEF 1) TOP 3 -4)
                                 (cons
                                  (make-ball 302 PADDLE-CTR-Y 3 4)
                                  empty)) 300))
              (make-game (cons (next-ball (make-ball (+ LEF 1) TOP 3 -4))
                               empty) 300))
...
(check-expect (next-balls (make-game
                           (cons (make-ball (+ LEF 1) TOP 3 -4)
                                 (cons (make-ball 269 PADDLE-CTR-Y 3 4)
                                       empty)) 300))
              (make-game (cons (next-ball (make-ball (+ LEF 1) TOP 3 -4))
                               (cons
                                (next-ball (make-ball 269 PADDLE-CTR-Y 3 4))
                                empty)) 300))

(check-expect (next-balls (make-game
                           (cons (make-ball (+ LEF 1) TOP 3 -4)
                                 (cons (make-ball 300 (+ PADDLE-CTR-Y 4) 3 4)
                                       empty)) 300))
              (make-game (cons (next-ball (make-ball (+ LEF 1) TOP 3 -4))
                               empty) 300))
...
(define (next-balls g)
  (make-game (game-with-next-balls (game-with-caught-balls
                                    (game-balls g)
                                    (game-paddle g)))(game-paddle g)))

; =============== HELPERS FOR next-balls ===============
; =============== HELPER game-with-next-balls ===============

(@htdf game-with-next-balls)
(@signature ListOfBall -> ListOfBall)

;(define (game-with-next-balls lob)) ;stub
...
; =============== HELPER game-with-caught-balls ===============

(@htdf game-with-caught-balls)
(@signature ListOfBall Number -> ListOfBall)

;(define (game-with-caught-balls lob)) ;stub
...
(check-random (handle-mouse (make-game empty PADDLE) 100 200 "button-down")
              (make-game (cons (make-ball 100 200 (- 5 (random 11))
                                          (- 5 (random 11)))
                               empty) PADDLE))
(check-random (handle-mouse (make-game
                             (cons (make-ball 10 20 -3 3) empty)
                             PADDLE)
                            300 100 "button-down")
              (make-game (cons (make-ball 300 100 (- 5 (random 11))
                                          (- 5 (random 11)))
                               (cons (make-ball 10 20 -3 3) empty)) PADDLE))
(check-random (handle-mouse (make-game empty PADDLE) 100 200 "button-up")
              (make-game empty PADDLE))
(check-random (handle-mouse (make-game
                             (cons (make-ball 10 20 -3 3) empty)
                             PADDLE)
                            100 200 "button-up")
              (make-game (cons (make-ball 10 20 -3 3) empty) PADDLE))
#;
...
(@template
 (define (handle-mouse g x y me)
   (cond [(mouse=? me "button-down") (... (game-balls g) x y)]
         [else
          (... (game-balls g) x y)])))
...
(check-expect (handle-key (make-game
                           (cons
                            (make-ball (/ WIDTH 2) (/ HEIGHT 2) 2 4)
...
(check-expect (handle-key (make-game
                           (cons
                            (make-ball (/ WIDTH 3) (/ HEIGHT 4) 1 -3)
...
(define (handle-key g ke)
  (cond [(key=? ke " ") (make-game empty (game-paddle g))]
        [(key=? ke "right") (if (< (+ (game-paddle g)(/ PADDLE-WIDTH 2))
                                   WIDTH)
                                (make-game
                                 (game-balls g)
                                 (+ (game-paddle g)
        ...
        [(key=? ke "left") (if (> (+ (game-paddle g)(/ PADDLE-WIDTH 2))
                                  PADDLE-WIDTH)
                               (make-game
                                (game-balls g)
                                (- (game-paddle g)
        ...
        [else (make-game (game-balls g) (game-paddle g))]))
